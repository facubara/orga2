...
void inic_game(){
	...
	jugador_t j2 = (jugador_t){
        ...
        int j;
        visitadasA[0]=0x851000;
        visitadasB[0]=0x800000 + 78 * 0x1000 + 42 * 80 * 0x1000;
        for(j=1; j<3520;j++){
                visitadasA[j] = 0;
                visitadasB[j] = 0;
        }
...
{    
	...
	//revisar_mapeadas(virtualDst, visitadas, jugadores[jugadorJugando].ult_indice_vis++)	
