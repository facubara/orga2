...
unsigned int run_blur(int c, char* src, char* dst, int iteraciones){
  ...
  if(*(bmp_get_bitcount(bmp)) == 24) {
	...
	unsigned long end;
	unsigned long delta;
	FILE* test = fopen("resultadosBLUR.txt","a");
...
unsigned int run_merge(int c, char* src1, char* src2, char* dst, float value, int iteraciones){
  ...
  int i;
	...
	unsigned long end;
	unsigned long delta;
	FILE* test = fopen("resultadosMERGE.txt","a");
	for (i = 0; i < iteraciones; i++){
...
unsigned int run_hsl(int c, char* src, char* dst, float hh, float ss, float ll, int iteraciones) {
  BMP* bmp = bmp_read(src);
  if(bmp==0) { return -1;}  // open error
  ...
  int i;
	...
	unsigned long end;
	unsigned long delta;
	FILE* test = fopen("resultadosHSL.txt","a");
	for (i = 0; i < iteraciones; i++){
